...
function altPickerHtml(s){
 ...
 return `<div class="alt-picker"><div class="alt-picker-head"><b>Alternate recipes · ${picked.size} selected</b><input id="alt-filter" type="search" placeholder="Filter by recipe or product…" aria-label="Filter alternate recipes"></div><p class="small muted">Only the recipes you tick are allowed in the plan. Hard-drive alternates are unlocked from crash sites; Turbofuel and Compacted Coal are researched in the MAM instead. Recipes your other choices depend on are selected automatically: a turbofuel-based power route locks its MAM recipes, and requiring pure ingots locks the pure recipes. Raw-resource conversion recipes are not alternates — they follow the SAM conversion setting and Tier 9 unlocks. Selecting none plans with standard recipes only.</p><div class="alt-list">${list.map(a=>{const outs=Object.keys(a.outputs);const why=a.mam&&!['auto','coal','fuel'].includes(s.mainPower||'auto')?'power preference':a.pure&&s.pureIngots===true?'ingot preference':'';return `<div class="alt-row" data-alt-text="${esc((a.name+' '+outs.join(' ')).toLowerCase())}"><label class="check-row">${why?`<input type="checkbox" checked disabled aria-label="${esc(a.name)} is required by your ${why}">`:`<input type="checkbox" name="alt" value="${esc(a.id)}" ${picked.has(a.id)?'checked':''}>`}<span>${esc(a.name)}<small class="muted"> · ${esc(outs.join(', '))} · ${a.mam?'MAM research':'Phase '+a.phase}${why?' · required by your '+why:''}</small></span></label><button type="button" class="btn quiet alt-info" data-alt-info="${esc(a.id)}" aria-label="Show the ${esc(a.name)} recipe">recipe ↗</button></div>`;}).join('')}</div></div>`;
}
