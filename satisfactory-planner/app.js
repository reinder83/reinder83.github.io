...
document.addEventListener('change',async e=>{
 ...
 if(['recipes','mainPower'].includes(el.name)&&wizard&&$('#wizard-form')){readWizard($('#wizard-form'));render();}
 if(el.name==='alt'){const p=el.closest('.alt-picker');const head=p?.querySelector('.alt-picker-head b');if(head)head.textContent=`Alternate recipes · ${p.querySelectorAll('input[name=alt]:checked').length} selected`;}
...
function altPickerHtml(s){
 ...
 return `<div class="alt-picker"><div class="alt-picker-head"><b>Alternate recipes · ${picked.size} selected</b><input id="alt-filter" type="search" placeholder="Filter by recipe or product…" aria-label="Filter alternate recipes"></div><p class="small muted">Only the recipes you tick are allowed in the plan. Hard-drive alternates are unlocked from crash sites; Turbofuel and Compacted Coal are researched in the MAM instead, and a power preference that runs on turbofuel or its Phase 3 bridge selects them automatically. Pure ingot recipes are added when required above. Selecting none plans with standard recipes only.</p><div class="alt-list">${list.map(a=>{const outs=Object.keys(a.outputs);const locked=a.mam&&!['auto','coal','fuel'].includes(s.mainPower||'auto');return `<div class="alt-row" data-alt-text="${esc((a.name+' '+outs.join(' ')).toLowerCase())}"><label class="check-row">${locked?`<input type="checkbox" checked disabled aria-label="${esc(a.name)} is required by your power preference">`:`<input type="checkbox" name="alt" value="${esc(a.id)}" ${picked.has(a.id)?'checked':''}>`}<span>${esc(a.name)}<small class="muted"> · ${esc(outs.join(', '))} · ${a.mam?'MAM research'+(locked?' · required by your power preference':''):'Phase '+a.phase}</small></span></label><button type="button" class="btn quiet alt-info" data-alt-info="${esc(a.id)}" aria-label="Show the ${esc(a.name)} recipe">recipe ↗</button></div>`;}).join('')}</div></div>`;
}
...
function readWizard(form){const f=new FormData(form),w=wizard,s=w.settings,oldPreset=s.purity+'|'+s.distribution;for(const [k,v]of f){if(k==='availablePowerMW')s.availablePowerGW=Number(v)/1000;if(k==='saveName')w.saveName=v;if(k==='profileName')w.name=v;else if(k.startsWith('limit:'))s.limits[k.slice(6)]=Number(v);else if(['utilityPercent','droneFuelRate','droneBridgeRate','multiplier','powerFactor','availablePowerGW','uraniumReactors','storageRate','cellsPerMinute','hours'].includes(k))s[k]=Number(v);else if(k==='collectables')s.collectables=v==='true';else if(k==='pureIngots')s[k]=v==='true';else if(['phase','purity','distribution','recipes','sam','nuclear','storage','goal','modNotes','mainPower','worldSeed','droneFuel'].includes(k))s[k]=v;}
 if(form.querySelector('.alt-list'))s.alternateRecipes=f.getAll('alt').map(String);
...
function calcWarnings(){const x=calcStage();return `${!x.feasible?`<div class="notice"><b>Planning draft — resource budget exceeded or recipe combination unavailable.</b> ${esc(x.reason)}</div>`:''}${x.additionalHeadroomMW>0.01?`<div class="notice">Allow another ${power(x.additionalHeadroomMW)} for whole-building power headroom. Phase 1 needs biomass or existing generation.</div>`:''}`;}
