...
const RULES=[
 ...
 {id:'guided',on:['wizard'],tone:'calm',when:f=>f.guided&&f.guidedStep<=f.guidedTotal,
  text:f=>`Question ${f.guidedStep} of ${f.guidedTotal}. The settings you are not being asked about keep their defaults, which is broadly the point of a default. All settings is one click away when you disagree.`},
 {id:'guided-supply',on:['wizard'],tone:'calm',when:f=>f.supplyDeclared>0,
  text:f=>`${plural(f.supplyDeclared,'line')} declared as already running. The plan will build the remainder and skip the chain behind them. It believes your rates exactly as literally as it believes your budgets.`},
 {id:'guided-tutorial',on:['wizard'],tone:'calm',when:f=>f.tutorialDone,
  text:()=>`The HUB tutorial is recorded as finished, so its steps start ticked. They are ticked, not deleted — untick one and it is back at the top of the list.`},
